/**
 * @deprecated This is no more used, please use either FullAsyncManager or StepByStepManager
 */
...
Manager.prototype.run = function() {
	...
	this.filter.on('accepted', function(/*Item*/item) {
	...
	this.filter.on('rejected', function(/*Item*/item) {
	...
	this.filter.on('done', function(/*Item*/item) {
		// apply next filter
		if(++idx<foundItems.length) {
			this.filter.accept(foundItems[idx], idx, foundItems);
		}
	}.bind(this));

	this.nameProvider.on('done', function(/*Item*/item) {
		winston.log("debug", "set download url for "+item.getName());
		this.urlProvider.setDownloadUrl(item);
	}.bind(this));

	this.urlProvider.on('done', function(/*Item*/item) {
		// start download
		winston.log("debug", "start download for "+item.getName());
		this.downloader.start(item);
	}.bind(this));

	this.downloader.on('started', function(/*Item*/item) {
	...
	this.downloader.on('downloaded', function(/*Item*/item) {
