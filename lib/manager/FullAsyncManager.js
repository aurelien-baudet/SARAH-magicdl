 ...
 */
function FullAsyncManager(/*Object*/sarahContext, /*Searcher*/searcher, /*Filter*/filter, /*NameProvider*/nameProvider, /*UrlProvider*/urlProvider, /*Downloader*/downloader, /*Player*/player, /*Notifier{}*/notifiers) {
	BaseManager.call(this, sarahContext, searcher, filter, nameProvider, urlProvider, downloader, player, notifiers);
	this.register();
}
...
FullAsyncManager.prototype.register = function() {
	...
	// progress
	this.downloader.on('progress', this.currentProgress.bind(this));
}
...
FullAsyncManager.prototype.rejected = function(/*Item*/item) {
	...
	if(++this.numRejected>=this.foundItems.length) {
		this.notifiers.nothing.notify(item);
	}
}
...
FullAsyncManager.prototype.downloadStarted = function(/*Item*/item) {
	...
	this.notifiers.downloadStarted.notify(item);
}
...
FullAsyncManager.prototype.downloaded = function(/*Item*/item) {
	...
	this.notifiers.downloaded.notify(item);
	this.play(item);
}
