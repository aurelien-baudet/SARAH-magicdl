...
controller.getDownloadFiles = function(task, next) {
	loginApp(prepareGetRequest('downloads/'+task+'/files', next));
}

/**
 * addFileDownload method
 *
 * Add one file download to the queue.
 *
 * Example :
 *
 * 	freebox.addFileDownload(
 *		"sdqfdsfds",
 *		null, null,
 *		function(msg) {
 *			console.log(msg);
 ...
 *	);
 * 
 * @param {object}   file             The file name, the content of the file and the content type
 * @param {string}   dir              The download destination directory (optional)
 * @param {string}   archive_password Pasword to decompress the erchive if nzb.
 * 
 */
function addFileDownload(file, dir, archive_password, next) {
	...
	if(app.permissions.downloader) {
		...
		var r = request(options, function (error, response, body) {
			...
			{
				flash("error on add downloads: "+body);
			}

		});
		var f = r.form();
		f.append('download_file', new Buffer(file.content),{
					filename: file.name,
					contentType: file.contentType,
//					knownLength: file.content.length
				});
		if(dir) {
			f.append('download_dir', dir);
		}
		if(archive_password) {
			f.append('archive_password', archive_password);
		}
...
controller.addFileDownload = function (file, dir, archive_password, next) {
	loginApp(function(){ addFileDownload(file, dir, archive_password, next) });
}; 
