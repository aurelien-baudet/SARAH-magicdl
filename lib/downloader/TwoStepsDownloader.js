...
TwoStepsDownloader.prototype.start = function(/*Item*/item) {
	...
	var request = http.get(item.getDownloadUrl(), function(file, response) {
	  response.pipe(file);
	}.bind(this, file));
	file.on('finish', function(item, torrentPath) {
		console.log("finish", item.getName(), torrentPath);
