	...
	freebox.downloads(null, null, null, function(downloads){
		...
		for(var i=0 ; i<this.downloading.length ; i++) {
			...
			if(fbdl && (fbdl.status=="done" || fbdl.status=="seeding")) {
				...
				// get the path to the real video file (in case the vdeo is in folder)
				this.getVideoFile((this.getDirectory(item) || '/Disque dur/Téléchargements/')+'/'+fbdl.name, function(path) {
					// share file in order to be able to play it
					freebox.share.create(path, function(share) {
						// update the play url
...
/**
 * Get the video file. If the path refers to a folder, then enter in it and search for video files.
 * If there are several files, then take the first one. If the first one is a sample, then take the next one.
 * 
 * @param path the path to a file or a folder (not encoded)
...
FreeboxDownloader.prototype.getVideoFile = function(/*String*/path, /*Function*/callback) {
	var normalizedPath = freebox.encodePath(path.replace(/\?.*$/, ""));
	// FIXME: freebox.fs.info doesn't work
//	freebox.fs.info(normalizedPath, function(result) {
	freebox.fs.ls(normalizedPath, function(files) {
		// NOTE: code to simulate fs.info api
		var result = {};
		if(files.length==1) {
			result.type = files[0].type;
		} else {
			result.type = "dir";
		}
		if(result.type=="dir") {
			freebox.fs.ls(normalizedPath, function(files) {
				...
				if(bestFile) {
					callback(bestFile.path);
				} else {
					winston.log("error", "no video file found in folder "+path);
				}
			});
		} else {
