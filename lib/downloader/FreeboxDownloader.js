...
FreeboxDownloader.prototype.download = function(/*Item*/item) {
	...
	if(item.getDownloadFile()) {
		...
		freebox.addFileDownload(item.getDownloadFile(), encodedDir, null, this.downloadStarted.bind(this, item));
	} else {
		winston.log("debug", "call freebox.addDownloads "+item.getName());
		freebox.addDownloads(item.getDownloadUrl(), encodedDir, false, null, null, null, this.downloadStarted.bind(this, item));
	}
}

FreeboxDownloader.prototype.downloadStarted = function(/*Item*/item, /*Object*/download) {
	...
	freebox.downloads(null, null, null, function(downloads){
		...
		for(var i=0 ; i<this.downloading.length ; i++) {
			...
			if(fbdl && (fbdl.status=="done" || fbdl.status=="seeding")) {
				...
				this.getVideoFile(fbdl.id, function(path) {
