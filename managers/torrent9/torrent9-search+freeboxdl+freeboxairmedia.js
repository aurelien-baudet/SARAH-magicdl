 ...
 */
function Torrent9SearchFreebox(sarahContext) {
...
util.inherits(Torrent9SearchFreebox, Manager);
 ...
 */
Torrent9SearchFreebox.initialize = function(initCtx) {
	...
	FreeboxDownloader.ee.once('done', function(appInfo) {
		if(appInfo) {
			...
			FreeboxAirMedia.ee.once('done', Torrent9SearchFreebox.ee.emit.bind(Torrent9SearchFreebox.ee, 'done'));
		}
	});
}

Torrent9SearchFreebox.ee = new EventEmitter();
 ...
 */
Torrent9SearchFreebox.detect = function(detectCtx) {
	new FreeboxDetector().detect().on('available', Torrent9SearchFreebox.ee.emit.bind(Torrent9SearchFreebox.ee, 'available'));
}


module.exports = Torrent9SearchFreebox;
